...
import static com.github.priytam.DelayedQueueService.delayedQueueService;
...
import com.github.priytam.context.DefaultEventContextHandler;
...
class DelayedEventServiceTest {
    ...
    private DelayedQueueService eventService;
    ...
    void setUp() throws IOException {
        ...
        eventService = delayedQueueService()
                .client(redisClient)
                .mapper(objectMapper)
    ...
    void shouldHandleDifferentEventsInParallel() {
        ...
        eventService.subscribe(DummyEvent.class, e -> Mono.fromCallable(() -> randomSleepBeforeCountdown(latch)), 3);
        eventService.subscribe(DummyEvent2.class, e -> Mono.fromCallable(() -> randomSleepBeforeCountdown(latch)), 3);
        eventService.subscribe(DummyEvent3.class, e -> Mono.fromCallable(() -> randomSleepBeforeCountdown(latch)), 3);
    ...
    void shouldBeAbleToReconnect() throws InterruptedException, IOException {
        // given
        eventService.subscribe(DummyEvent.class, DUMMY_HANDLER, 1);
    ...
    void shouldFailToSubscribeIfConnectionNotAvailable() throws IOException {
        ...
        assertThrows(RedisConnectionException.class, () -> eventService.subscribe(DummyEvent.class, DUMMY_HANDLER, 1));
    }
    ...
    void shouldFailToDispatchIfConnectionNotAvailable() throws IOException {
        ...
        assertThrows(RedisCommandTimeoutException.class, () -> eventService.dispatch());
    }
    ...
    void shouldRescheduleEventForLaterTimeDuringDispatch() {
        ...
        double initialScore = connection.zscore(DELAYED_QUEUE, DelayedQueueService.getKey(event));
        // when
        eventService.dispatch();
        double postDispatchScore = connection.zscore(DELAYED_QUEUE, DelayedQueueService.getKey(event));
    ...
    void shouldBeAbleToRemoveHandler() {
        ...
        waitAndAssertEventsCount(0L);
        // when
        assertThat(eventService.unsubscribe(DummyEvent.class), equalTo(true));
        assertThat(eventService.unsubscribe(DummyEvent.class), equalTo(false));
    ...
    void shouldValidateAddedHandler() {
        assertThrows(NullPointerException.class, () -> eventService.subscribe(null, DUMMY_HANDLER, 1));
        assertThrows(NullPointerException.class, () -> eventService.subscribe(DummyEvent.class, null, 1));
        assertThrows(IllegalArgumentException.class, () -> eventService.subscribe(DummyEvent.class, DUMMY_HANDLER, 0));
        assertThrows(IllegalArgumentException.class, () -> eventService.subscribe(DummyEvent.class, DUMMY_HANDLER, 101));
    }
    ...
    private Mono<Void> enqueue(IntStream stream, Function<Integer, Event> transformer) {
        return Flux
                ...
                .flatMap(id -> eventService.enqueue(transformer.apply(id), Duration.ZERO))
                .then();
    }
